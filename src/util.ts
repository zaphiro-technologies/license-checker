...
export function matchesPath(path: string, patterns: string[] | undefined): boolean {
  ...
  return patterns.some((pattern) => {
    ...
    return (
      ...
      minimatch(normalized, `${candidate.replace(/\/$/, '')}/**`, { dot: true }) ||
...
export function listRepositoryFiles(root: string): string[] {
  try {
    ...
    const files: string[] = [];
    const ignored = new Set(['.git', 'node_modules', 'vendor', 'dist', 'build', '.venv', 'venv']);
    ...
    const walk = (directory: string): void => {
      for (const entry of readdirSync(directory, { withFileTypes: true })) {
        ...
        else if (entry.isFile()) files.push(pathForGlob(relative(root, absolute)));
      }
    };
