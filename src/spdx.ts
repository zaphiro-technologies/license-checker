...
export type Compatibility = 'compatible' | 'weak-compatible' | 'incompatible' | 'unknown';

const CATEGORY_A = new Set([
  'Apache-2.0', 'PHP-3.01', '0BSD', 'BSD-3-Clause', 'BSD-2-Clause', 'BSD-2-Clause-Views',
  'PostgreSQL', 'EDL-1.0', 'ISC', 'SMLNJ', 'ICU', 'NCSA', 'W3C', 'Xnet', 'Zlib', 'Libpng',
  'AFL-3.0', 'MS-PL', 'Python-2.0', 'BSL-1.0', 'WTFPL', 'Unicode-DFS-2016',
  'Unicode-DFS-2015', 'ZPL-2.0', 'Unlicense', 'HPND', 'MulanPSL-2.0', 'MIT', 'MIT-0',
]);

const CATEGORY_B = new Set([
  'CDDL-1.0', 'CDDL-1.1', 'CPL-1.0', 'EPL-1.0', 'EPL-2.0', 'ErlPL-1.1', 'IPA', 'IPL-1.0',
  'UFL-1.0', 'UnRAR', 'MPL-1.0', 'MPL-1.1', 'MPL-2.0', 'OFL-1.1', 'OSL-3.0', 'Ruby',
  'SPL-1.0',
]);
...
const CATEGORY_A_INCOMPATIBLE = new Set([
  ...
  'LGPL-3.0+', 'LGPL-3.0', 'LGPL-3.0-linking-exception', 'LGPL-3.0-only', 'LGPL-3.0-or-later',
  'LGPLLR', 'GPL-1.0+', 'GPL-1.0', 'GPL-1.0-only', 'GPL-1.0-or-later',
  ...
  'GPL-3.0+', 'GPL-3.0', 'GPL-3.0-linking-exception', 'GPL-3.0-linking-source-exception',
  'GPL-3.0-only', 'GPL-3.0-or-later', 'GPL-3.0-with-autoconf-exception', 'GPL-3.0-with-GCC-exception',
  'GPL-CC-1.0', 'QPL-1.0', 'Sleepycat', 'SSPL-1.0', 'CPOL-1.02', 'NPL-1.0', 'NPL-1.1',
]);
...
const OSI_APPROVED = new Set([
  ...
  'LGPL-2.0-only', 'LGPL-2.0-or-later', 'LGPL-2.1-only', 'LGPL-2.1-or-later',
  'LGPL-3.0-only', 'LGPL-3.0-or-later', 'AGPL-3.0-only', 'AGPL-3.0-or-later',
  'BSD-3-Clause-Clear', 'BSD-3-Clause-LBNL', 'BSD-2-Clause-Patent', 'EUPL-1.2',
]);
...
export function normalizeLicenseId(value: string): string {
  ...
  if (!trimmed || /^unknown$/i.test(trimmed) || /^noassertion$/i.test(trimmed)) return 'Unknown';
...
function convertParsed(value: unknown): SpdxExpression {
  const node = value as { license?: string; exception?: string; left?: unknown; right?: unknown; conjunction?: string };
  if (node.license && node.exception) {
    return { type: 'with', license: { type: 'license', id: normalizeLicenseId(node.license) }, exception: node.exception };
  }
  if (node.license) return { type: 'license', id: normalizeLicenseId(node.license) };
...
export function parseLicenseExpression(value: string | undefined): SpdxExpression {
...
export function normalizeLicenseExpression(value: string | undefined): string {
  ...
  const render = (node: SpdxExpression): string => {
    ...
    if (node.type === 'with') return `${render(node.license)} WITH ${node.exception}`;
...
function compareSingle(main: string, dependency: string, weakCompatible: boolean, config?: DependencyConfig): Compatibility {
  ...
  if (CATEGORY_A.has(main)) {
    ...
    if (CATEGORY_B.has(dependency)) return weakCompatible ? 'weak-compatible' : 'unknown';
  ...
  if (CATEGORY_B.has(main)) {
    ...
    if (CATEGORY_A.has(dependency)) return weakCompatible ? 'weak-compatible' : 'unknown';
    return 'incompatible';
  }
...
function combineAnd(left: Compatibility, right: Compatibility): Compatibility {
  ...
  if (left === 'weak-compatible' || right === 'weak-compatible') return 'weak-compatible';
  return 'compatible';
}
